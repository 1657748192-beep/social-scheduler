...
- 内容编辑器：先按具体社媒账号多选，再编辑基础/平台版本文案、可选网站链接与表情/特殊符号、共用或平台专属的图片/视频素材；支持立即发布、北京时间 24 小时制定时发布和保存草稿。
...
- 内容编辑器：可勾选同一平台下的多个具体账号；每个已选账号生成独立变体/发布任务，支持基础文案、平台版本编辑、可选网站链接、表情/特殊符号插入、共用素材及平台专属素材调整和发布摘要。
...
- 内容编辑器中图片/视频上传入口。
- 上传后预览缩略图。
- 上传支持两路并行、单文件进度、失败原因和手动重试；单个图片/视频上限为 250 MB。
- 素材支持基础/平台版本：共用素材上传一次即可复用到 Instagram、Facebook、YouTube 等已选平台；任一平台可复制共用素材后独立增删或追加，不影响其他平台；同一平台的多个已选账号共享该平台最终素材。
- 内容预览中展示图片/视频。
- 发布任务可关联素材。
