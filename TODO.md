...
- 当前状态：已实现账号选择界面、立即/定时/草稿三种发布方式、账号归属校验和按账号发布；侧栏通道数按已连接平台种类统计、同平台账号数量不限；文案支持按基础/平台分别设置可选网站链接及表情/特殊符号插入；素材支持共用上传、按平台复制后独立调整，同一平台的多个账号共享该平台最终素材；已添加数据库外键迁移，待线上用多个 Facebook Page / YouTube Channel 做真实发布验证。
...
- 当前状态：基础上传和预览已完成；支持两路并行上传、进度、失败原因和重试，单文件上限 250 MB。已实现腾讯云 COS 私有桶直传、短期 STS 凭证、短期签名预览/发布 URL、旧本地素材迁移命令和 Worker 自动清理。上线前需在腾讯云配置私有桶/CORS/CAM，并在服务器 `.env` 设置 `MEDIA_STORAGE=cos`；详见 `docs/COS_MEDIA_SETUP.md`。
- 完成标准：客户能高效管理大量素材。
- 依赖条件：存储方案确定。
